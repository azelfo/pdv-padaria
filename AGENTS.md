...
- **Config:** `PdvPadaria/.env` (gitignored) supplies six keys — `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `TENANT_ID`, `STORE_SYNC_TOKEN`, `TERMINAL_NAME`, and an optional `STORE_ID`. Read via `Services/EnvService.cs`, which treats a **blank value as absent** so the caller's fallback applies (`.env.exemplo` ships the keys empty).
  ...
  **Switching a machine to another store wipes its local transactional data first** (`SyncService.PrepararTrocaDeLoja`, then a pull with `semearEstoqueDaNuvem: true`). The credential switches at login, but the SQLite file holds the STOCK — without the wipe, the first sync publishes the old store's balance as the new store's. The switch is refused while any sale or movement is still unsent, since those belong to the previous store and only upload under its credential.
